...
3. Optional: If using a CDM 8.x.x AMI or earlier, set the `metadata_http_put_response_hop_limit` variable to `1`.
   The following is expected if this condition is not met:
    ...
    │ 
    │   on ../../terraform-aws-rubrik-cloud-cluster-elastic-storage/variables.tf line 311, in check "imds_hop_count":
    │  311:     condition     = !contains(var.aws_ami_filter, "rubrik-mp-cc-8*") || var.metadata_http_put_response_hop_limit == 1
    │     ├────────────────
    │     │ var.aws_ami_filter is set of string with 1 element
    │     │ var.metadata_http_put_response_hop_limit is 2
    │ 
    │ When using a CDM 8.x.x AMI the instance metadata option defaults to 1. 
...
| <a name="requirement_polaris"></a> [polaris](#requirement\_polaris) | >= 1.1.2 |
